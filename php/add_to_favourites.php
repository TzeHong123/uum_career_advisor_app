...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($insertSql->execute()) {
            echo json_encode(['status' => 'success', 'message' => 'Post added to favorites successfully.']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Error adding post to favorites: ' . $conn->error]);
        }
    }
} else {
